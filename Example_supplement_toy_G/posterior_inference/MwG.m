...
for i=1:n_simu
    ...
    ss=Simulator(Z_new,data)*S_R1;
    ERR_emu(i)=mean((ss-emulator_Z_new).^2);
...
logprior=-(data.para_prior.a+M/2)*log(sigmasqf)-(Z*(invR_Z_eta*Z')/(2*sigmasqf)+data.para_prior.b);
loglikelihood=-sum((data.obsp-emulator_Z).*(Sigma_e').^(-1).*(data.obsp-emulator_Z))/2-logdetSig/2;
