...
[True,data]=Pde_parameter(0.5,zeros(1,31),sigma_e,1);
data=generate_basis_all(data,1,size(data.X,1));
pre_full.data=data;
function y=preparition_for_general_data(sigma_e,M,basis,iso)
[True,data]=Pde_parameter(0.5,zeros(1,21),sigma_e,iso);
...
candidate_sample=Sample_from_prior_t(data,para_MCMC);%2500 candidate
...
para_design=Generate_Design(data,para_design);
...
%%
function chain=Sample_from_prior_t(data,para_MCMC)
% check 3
...
%     if rem(i,5000)==0
%         str=[num2str(floor((n_simu-i)/1000)) 'K steps and ' num2str(floor(time_used*(n_simu-i)/i)), ' seconds left'];
%         waitbar(i/n_simu,h,str);
%     end
end
chain=chain(1000:20:end,1:M);
% close(h);
