function [para_design,para_emulator]=fully_sequential_generation(data,para_emulator,para_design,z_initial,criteria)
% d_desire: size of follow-up design size design size
% criteria: indicate the design criteria used in simulation, 1: WPV; 2: VL; 3: AEI;
%z_initial: stating point of optimization
%%
z_post=z_initial;
...
while q_1<d_desire
    ...
    [z_add,para_emulator]=sequantially_add_one_point(data,para_emulator,z_post,criteria);
end
para_design.D_para=[para_emulator.D_para];
end

function [z_add,para_emulator]=sequantially_add_one_point(data,para_emulator,z_post,criteria)
%%
global optimal_design
optimal_design.para_emulator=para_emulator;
optimal_design.data=data;
        ...
        if data.fix_eta==0
            if data.isotropic==0
                ...
                if max(abs(z_add))>9.9
                    options_patternsearch = optimoptions('patternsearch','Display','off','MaxIterations',1000*data.M);
