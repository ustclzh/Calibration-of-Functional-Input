...
x_max=1000;
y_max=1000;
T=exp(-6); %transmissivity 
S=exp(-9); %specific storage
r=0.001;
time=0:100:10000;
time_obs=21:20:length(time);
hmax=50; % the largest mesh length allowed in generating mesh
[x_obs,y_obs]=meshgrid([0.25:0.25:0.75]); %observation location, i.e. position of observation wells
x_obs=x_obs*x_max;
y_obs=y_obs*y_max;
...
%max(True.f_input)-min(True.f_input);
% [xx,yy]=meshgrid(0:0.01:1);
% x_plot=xx*x_max;
% y_plot=yy*y_max;
% C= scatteredInterpolant(data.X',data.Y',True.f_input');
% c0=C(x_plot,y_plot);%this is the setup of coefficient "c"
%contour((0:0.01:1)*x_max,(0:0.01:1)*y_max,c0,'ShowText','on')
%imagesc([0,x_max],[0,y_max],c0)
