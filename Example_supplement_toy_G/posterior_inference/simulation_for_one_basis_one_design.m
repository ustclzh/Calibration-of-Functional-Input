...
data=pre.data;
z_true=((data.KL'*data.KL)\(data.KL'*True.f_input'))';
...
[para_design_adv,para_emulator_adv,c_trace,Z_mode]=fully_sequential_generation(data,para_emulator,para_design,z_initial,design_criterion);
%Candidate_for_start=[start_set;para_design_adv.D_para;Z_mode];
Candidate_for_start=[start_set;para_design_adv.D_para];
start_set=generate_design_support_points(n_batch_start,Candidate_for_start,100);
start_set=[start_set;z_initial];
%start_set=[start_set;result.x_opt(1:data.M)];
[para_emulator_adv,data]=Train_Emulator(data,para_design_adv,para_emulator_adv);
result=H2P(data,para_emulator_adv,start_set);
true=True.f_input;
z_initial=((data.KL'*data.KL)\(data.KL'*true'))';
z_post=result.x_opt;
f_post=z_post(1:data.M)*data.KL';
err_mean2=(mean((true-f_post).^2))^(1/2)/(mean(true.^2)^(1/2));
[m,~]=emulator(z_post(1:data.M),para_emulator_adv);
RSS=mean((m-data.obs).^2)^(1/2);
[m,~]=emulator(z_initial,para_emulator_adv);
RSS_o=mean((m-data.obs).^2)^(1/2);
result.post_err=err_mean2;
result.z_project=z_initial;
result.z_true=True.z_true;
result.prior_err=mean((z_initial*data.KL'-true).^2)^(1/2)/(mean(true.^2)^(1/2));
result.f_post=f_post;
result.para_design=para_design_adv;
result.c_trace=c_trace;
result.obs=data.obs;
result.eta_true=True.eta_true;
%result.True=True;
result.f_true=true;
result.f_proj=z_initial*data.KL';
RES_opt{1}=result;
[RSS,RSS_o,result.prior_err,err_mean2];%list result
%% MCMC part
if nargin==6
start_point=result.x_opt(1:data.M);
